...
__author__ = 'Zachary Gallant'
__student_number__ = '101272210'
...
class ArrayBag:
    ...
    def remove(self, item: any) -> any:
        ...
        if len(self._elems) == 0:
            ...
            while item_location == -1:
                if item == self._elems[count]:
                    item_location = count
                count += 1

        self._elems[item_location:self._num_items - 1] \
            = self._elems[item_location + 1:self._num_items]
        self._num_items -= 1
    ...
    def grab(self) -> any:
        ...
        if self._num_items == 0:
            ...
            random_num = random.randint(0, self._num_items - 1)
            removed_value = self._elems[random_num]
        return self.remove(removed_value)
    ...
    def __eq__(self, other: 'ArrayBag') -> bool:
        ...
        if len(other) == len(self):
            while i <= self._num_items - 1:
                list1 = list1 + [self._elems[i]]
                list2 = list2 + [other._elems[i]]
                i += 1
    ...
    def __mul__(self, n: int) -> 'ArrayBag':
        newBag = ArrayBag([])
        for i in range(n):
            for item in self:
                newBag.add(item)
