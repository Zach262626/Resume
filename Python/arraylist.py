...
class ArrayList:
    ...
    def extend(self, iterable) -> None:
        ...
        arr = _new_array(max(1, len(self._elems) + len(iterable)))
        arr[0:self._num_items] = self._elems[0:self._num_items]
        if self._num_items != int:
            self._num_items = 0
        self = arr
        for i in range(len(iterable)):
            self[self._num_items] = iterable._elems[i]
            self._num_items += 1
        """
    ...
    def pop(self, i: int = -1) -> any:
        ...
        """
        if len(self) == 0:
            IndexError("ArrayList: pop from empty list")
        if (i <= -1):
            i = (len(self)) + i
        if (i < 0) or (i >= len(self)):
            IndexError("ArrayList: pop index out of range")
        removed_item = self[i]
        del self[i]
        return removed_item

    def identical(self) -> (bool, int):
        """
        >>> lst = ArrayList([1, 4, 3, 6])
        >>> lst.identical()  
        False

        >>> lst = ArrayList([1, 4, 3, 6, 1])
        >>> lst.identical()
        True

        >>> lst = ArrayList([7, 3, 0, 3, 1])
        >>> lst.identical()
        True
